...
public class MySqlImportAndExport {  
    ...
    private static String getExportCommand(Properties properties) {  
        ...
        //注意哪些地方要空格，哪些不要空格  
        command.append("classpath:db/mysqldump -u").append(username).append(" -p").append(password)//密码是用的小p，而端口是用的大P。  
    ...
    private static String[] getImportCommand(Properties properties) {  
        ...
        //第一步，获取登录命令语句  
        String loginCommand = new StringBuffer().append("classpath:db/mysql -u").append(username).append(" -p").append(password).append(" -h").append(host)  
